...
//
// Deliberately pending-toned copy and iconography (never a green
// checkmark/"all set" wording): landing here only means the candidate
// finished on the hosted checkout page, never that the backend has
// confirmed payment -- that callback may still be in flight.
export default function PaymentPendingPage() {
  const { t } = useLanguage();

  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#F8F9FA] px-6 text-center">
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#FFF7E6] text-[#B45309]">
        <Clock size={28} />
      </div>
