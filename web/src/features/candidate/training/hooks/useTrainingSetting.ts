...
/** Fetches the one shared training link -- mirrors useCandidateFlightDetail.ts's identical retry/error-handling rationale. Explicit generics type the error as TrainingSettingError rather than TanStack Query's default `Error`, since TrainingPanel reads `.code` off it. */
