...
export interface UseShortLivedAccessResult<TAccess, TError = CandidateFlightDetailError> {
  access: TAccess | null;
 ...
 *
 * `TError` defaults to `CandidateFlightDetailError` (this hook's original,
 * flight-ticket-only caller) but any candidate document-access feature with
 * its own error taxonomy (e.g. visa copy access) can supply its own.
 */
export function useShortLivedAccess<
  TAccess extends { expiresAt: string },
  TError = CandidateFlightDetailError,
>(): UseShortLivedAccessResult<TAccess, TError> {
  ...
  const requestAccess = useCallback(
    async (fetchAccess: () => Promise<TAccess>) => {
      ...
      try {
        ...
        if (requestVersion !== requestVersionRef.current) return;
        setError(requestError as TError);
      } finally {
