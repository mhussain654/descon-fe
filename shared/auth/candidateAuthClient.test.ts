...
describe('createMockCandidateAuthClient', () => {
  ...
  it('verifyOtp succeeds with the documented mock code and returns a session', async () => {
    ...
    const session = await client.verifyOtp(CNIC, MOCK_VALID_OTP);
    expect(session.accessToken).toEqual(expect.any(String));
    expect(session.refreshToken).toEqual(expect.any(String));
    expect(session.candidateId).toEqual(expect.any(String));
    expect(session.candidateName).toEqual(expect.any(String));
    expect(session.preferredLocale).toEqual(expect.any(String));
  ...
  it('locks out after MOCK_MAX_ATTEMPTS incorrect codes', async () => {
    ...
    await expect(client.verifyOtp(CNIC, MOCK_VALID_OTP)).rejects.toEqual({
  ...
  it('verifyOtp rejects an expired challenge with OTP_EXPIRED', async () => {
    ...
    try {
      ...
      await expect(client.verifyOtp(CNIC, MOCK_VALID_OTP)).rejects.toEqual({ code: 'OTP_EXPIRED' });
    } finally {
      Date.now = originalNow;
    }
  });

  it('verifyOtp rejects a CNIC with no outstanding challenge with CHALLENGE_NOT_FOUND', async () => {
    const client = createMockCandidateAuthClient({ delayMs: 0 });
    await expect(client.verifyOtp('9999999999999', MOCK_VALID_OTP)).rejects.toEqual({
  ...
  it('resendOtp enforces the cooldown and reports remaining seconds', async () => {
    ...
    try {
      ...
      await expect(client.resendOtp(CNIC)).rejects.toEqual({
        code: 'RESEND_COOLDOWN',
        retryAfterSeconds: challenge.resendAfterSeconds - 5,
      });

      now += challenge.resendAfterSeconds * 1000; // cooldown elapsed
      const resent = await client.resendOtp(CNIC);
      expect(resent.expiresInSeconds).toBe(challenge.expiresInSeconds);
    } finally {
      Date.now = originalNow;
    }
  });
  ...
  it('resendOtp resets the attempt count, so a fresh code gets fresh attempts', async () => {
    ...
    try {
      ...
      const session = await client.verifyOtp(CNIC, '000000').catch((error) => error);
